...
def main(cfg):
    ...
    if getattr(cfg, 'run_experiments', True):
        # run only reward-scaling experiments (Q5ii) for theta=90 across 15 seeds
        # default seeds (15 seeds used across other scripts)
        default_seeds = [44, 63, 11, 15, 90, 21, 0] # 12, 52, 8, 42, 32, 31, 10, 98]
        seeds = default_seeds
        ...
        try:
            for seed in seeds:
                ...
                # Q5ii: Reward scaling experiment for theta=90 with scales 10.0 and 0.1
                theta = 90
                reward_scales = [10.0, 0.1]

                for reward_scale in reward_scales:
                    # 1) Fixed alpha = 0.05 (store under alpha_type 'fixed_alpha')
                    cfg.seed = int(seed)
                    ...
                    run_dir = os.path.join(project_root, 'exp', 'scale_sweep', alpha_type, f'scale_{reward_scale}', f'seed_{seed}')
                    os.makedirs(run_dir, exist_ok=True)
                    print(f"Starting run theta={theta} scale={reward_scale} alpha=fixed(0.05) seed={seed} -> {run_dir}")
                    os.chdir(run_dir)
                    try:
                    ...
                    # 2) Automated alpha tuning (store under alpha_type 'auto')
                    cfg.seed = int(seed)
